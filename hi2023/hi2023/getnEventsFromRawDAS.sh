...
for value in {0..1}
do
  queryName='dasgoclient --query="dataset dataset=/HIEphemeralZeroBias'$value'/HIRun2023A-v1/RAW | sum(dataset.nevents)" '
...
for value in {0..2}
do
  queryName='dasgoclient --query="dataset dataset=/HIForward'$value'/HIRun2023A-v1/RAW | sum(dataset.nevents)" '
  result=$(eval "$queryName")
  num=$(echo -e $result | tr -c -d 0-9)
...
for value in {0..3}
do
  queryName='dasgoclient --query="dataset dataset=/HIMinimumBias'$value'/HIRun2023A-v1/RAW | sum(dataset.nevents)" '
  result=$(eval "$queryName")
  num=$(echo -e $result | tr -c -d 0-9)
...
for value in {0..31}
do
  queryName='dasgoclient --query="dataset dataset=/HIPhysicsRawPrime'$value'/HIRun2023A-v1/RAW | sum(dataset.nevents)" '
  result=$(eval "$queryName")
  num=$(echo -e $result | tr -c -d 0-9)
